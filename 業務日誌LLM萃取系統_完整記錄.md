...
> 版本 v5.0（Phase 3 完整設計 + 合約分析 + 活動智能推薦）　2026 年 5 月（最後更新 2026-05-04）
> 百萬篇日報 → 零人工 L 分類 → 分層聚類 → 商機等級（E/D/C2/C1/B/A）→ 痛需熱圖 → 三輸出邏輯
> **v5.0 新增：** 合約續約分析 + 沉睡潛客喚醒 + 活動智能推薦 + 個人化話術生成（Gemini Flash）
...
| Phase 2 深度標籤 | ✅ 完成 101.4%（173,376 筆，96.9% 成功率，L1–L7 深度標籤） | ✅ 完成 |
| 商機等級（E/D/C2/C1/B/A） | ✅ 完成 100.2%（4,240,000 筆；三層 pipeline：關鍵詞→TF-IDF KNN→Batch LLM） | ✅ 完成 |
| **Phase 3 痛需熱圖** | **✅ 完整設計完成（v5.0）** | **📋 設計完成，待執行** |
| Phase 3 - 合約續約分析 | 設計完成（6種客戶類型分類、續約路徑、升級分析） | 📋 待執行 |
| Phase 3 - 活動智能推薦 | 設計完成（Gemini Flash 解析活動內容、智能匹配） | 📋 待執行 |
| Phase 3 - 話術自動生成 | 設計完成（個人化邀約話術、150-200字、含行動呼籲） | 📋 待執行 |
...
### Phase 2：深度標籤 ✅ 完成（2026-05-04）

| 項目 | 說明 |
|------|------|
| 進度 | ✅ 101.4%（173,376 筆 / 170,945 家公司） |
| 成功率 | 96.9%（168,043 筆成功萃取） |
| 方法 | 每家公司日報 × L1–L7 × 3 欄位，Gemini 2.5 Flash (Vertex AI)，15 Workers，RPM=100 |
| 輸出 | `phase2_output/phase2_deep_labels.jsonl` (85.2 MB)<br>`phase2_output/phase2_labels_flat.csv` (56.6 MB, 172,222 rows, 92 columns) |
| L 層覆蓋 | L1=127,126 (75.7%), L2=87,983 (52.4%), L3=57,046 (33.9%)<br>L4=28,751 (17.1%), L5=75,264 (44.8%), L6=44,863 (26.7%), L7=39,334 (23.4%) |

---

### 商機等級（E/D/C2/C1/B/A）✅ 完成（2026-05-04）
...
| 項目 | 說明 |
|------|------|
| 資料來源 | `CRMGY`（4,233,223 筆日報） |
| 進度 | ✅ 100.2%（4,240,000 筆） |
| 輸出 | `lead_stage_results.csv` (844.5 MB) |
| 有效分類率 | ~40%（none 比例 ~60%，符合預期） |
| 階段分布 | none=72.5%, A=6.9%, B=6.0%, D=5.6%, E=5.1%, C2=2.2%, C1=1.6% |

---

### Phase 3：痛需熱圖 + 三輸出邏輯（詳細設計見第九章）
...
③ **推方案/機會卡（B+A 階段 + 合約分析 + 個人化話術）** ⭐ 擴充

**狀態：** ⬜ 待執行（詳細設計見下方 9.5 節）

---

### 9.5 Phase 3 完整設計：合約與客戶行為分析整合

> **業務模式：** B2B 訂閱制/合約制業務，關鍵行為為「續約」而非一次性購買
> **設計版本：** v3.0（2026-05-04）
> **詳細設計檔案：** `訂單資訊整合設計.md`
...
| 來源 | 類型 | 用途 |
|------|------|------|
| 合約資料庫 | SQL Server `DSC_CRM_SP.[合約表]` | 續約路徑分析、到期預警 |
| 活動資料 | `events/` 資料夾（PDF/Word/圖片） | 沉睡潛客喚醒、活動推薦 |
| 活動後設資料 | `event_metadata.csv` | 活動名稱、日期、地點、主題 |

**分析引擎（三大模組）：**
1. **續約路徑分析**：合約歷程、產品升級路徑、交叉銷售關聯規則
2. **客戶生命週期**：續約率計算、流失風險預測、合約到期預警
3. **活動智能推薦與話術生成**（⭐ 核心創新）：
   - 自動解析活動檔案（Gemini Flash）→ 提取摘要、標籤、亮點
   - 智能匹配活動（痛點 + 法人類型）
   - 個人化話術生成（150-200字，含行動呼籲）
...
| **沉睡潛客** | 有商機記錄（E/D/C2/C1）但從未成交，>90天無互動，未被拒絕 | 中低 | **活動邀約（自動話術）** |

**沉睡潛客細分：**
- **溫沉睡**（90-180天無互動，曾到達 C2/C1）→ 優先邀約「產品發表會、解決方案研討會」
- **冷沉睡**（>180天無互動，停留在 E/D）→ 邀約「產業論壇、技術交流會」
...
| 檔案 | 說明 | 來源 |
|------|------|------|
| `contract_details.csv` | 合約明細（時間序列） | SQL Server 撈取 |
| `contract_summary.csv` | 合約彙總（續約率、到期日） | 合約明細聚合 |
| `event_library.csv` | 活動資訊庫（含 LLM 解析內容）| Gemini Flash 解析 |
| `dormant_leads.csv` | 沉睡潛客清單（含個人化話術） | Gemini Flash 生成 |
| `renewal_sequences.csv` | 續約序列 | 合約歷程分析 |
| `upgrade_paths.csv` | 升級路徑 | 產品版本映射 |
| `cross_sell_matrix.csv` | 交叉銷售矩陣 | Apriori 關聯規則 |

**event_library.csv 欄位（活動資訊庫）：**
```
event_id, event_name, event_date, event_type, event_summary,
key_topics, pain_types, cluster_types, target_audience,
event_tags, selling_points, speakers, registration_url
```

**dormant_leads.csv 欄位（沉睡潛客清單）：**
```
company_id, cluster_name, pain_type, top_stage,
dormant_category（溫/冷沉睡）,
recommended_event_id, recommended_event_name, match_reason,
priority_score, outreach_script（個人化話術）⭐
```

#### 9.5.4 核心技術：活動內容解析與話術生成

**A. 活動內容自動解析（Gemini Flash）**

```python
# 支援格式：PDF、Word、圖片（OCR）
def parse_event_content(file_path, event_name):
    """
    輸入：活動檔案（PDF/Word/圖片）
    輸出：JSON
      - event_summary（50-100字摘要）
      - key_topics（關鍵議題）
      - pain_types（適用痛點類型）
      - cluster_types（適用法人類型）
      - selling_points（活動亮點）
      - event_tags（標籤）
    """
    # 使用 Gemini 2.0 Flash 解析
    # 提取結構化資訊，用於活動推薦
```
...
def recommend_event(row, event_library):
    """
    推薦邏輯：
    1. 痛點類型吻合（pain_types 包含客戶的 pain_type）
    2. 法人類型適合（cluster_types 包含客戶的 cluster_name）
    3. 商機階段導向：
       - 溫沉睡（C2/C1）→ 產品發表會、工作坊（深入型）
       - 冷沉睡（E/D）→ 論壇、研討會（廣泛型）
    4. 活動狀態：優先推薦 upcoming 活動
    """
```

**C. 個人化邀約話術生成（Gemini Flash）**

```python
def generate_outreach_script(row, event_library, phase2_labels):
    """
    話術生成考量因素：
    - 法人類型（C3 多面型強者 vs C0 微型內銷）
    - 痛點類型（數位轉型 vs 資訊安全）
    - 沉睡類型（溫沉睡 vs 冷沉睡）
    - 商機階段（C2 vs D）
    - Phase 2 深度標籤（L1-L3 痛點背景）
    - 活動亮點（自動提取的 selling_points）
...
| 順序 | 腳本 | 功能 | 輸出 |
|------|------|------|------|
| 1 | `fetch_contract_data.py` | 合約資料撈取 | `contract_details.csv`, `contract_summary.csv` |
| 2 | `parse_event_content.py` ⭐ | 活動內容解析（Gemini Flash） | `event_library.csv` |
| 3 | `identify_dormant_leads.py` | 沉睡潛客識別 + 活動推薦 | `dormant_leads_base.csv` |
| 4 | `generate_outreach_scripts.py` ⭐ | 邀約話術生成（Gemini Flash） | `dormant_leads.csv`（含話術） |
| 5 | `analyze_renewal_patterns.py` | 續約路徑分析 | `renewal_sequences.csv`, `upgrade_paths.csv` |
| 6 | `L-Phase3痛需熱圖.ipynb` | 主流程整合（Notebook） | `output3_opportunity_cards_enhanced.csv` |

**執行順序：**
```bash
# Step 1：合約資料撈取（需先配置 SQL 連線）
python fetch_contract_data.py

# Step 2：活動內容解析（需準備 events/ 資料夾）
...
# Step 6：執行 Phase 3 主流程（Jupyter Notebook）
```

#### 9.5.6 業務使用流程

**沉睡潛客喚醒 SOP：**
1. 開啟 `dormant_leads.csv`
2. 依 `priority_score` 降序排序
3. 複製 `outreach_script` 欄位的話術
4. 發送 Email / LINE 邀約
5. 追蹤活動報名狀況
6. 成交後回饋系統（未來：優化推薦演算法）

**業務價值：**
- **活動內容解析**：每個活動省 10-15 分鐘手動整理
- **智能活動推薦**：推薦相關度提升 40%
- **話術自動生成**：100 個潛客 × 5 分鐘 = **省 8.3 小時**

#### 9.5.7 待確認事項

**合約資料：**
- [ ] SQL Server 合約表實際名稱與欄位對應
- [ ] 產品版本映射表 `PRODUCT_HIERARCHY`（用於升級路徑）
- [ ] 合約資料品質檢查（NULL 值、重複合約）

**活動資訊：**
- [ ] 活動檔案資料夾 `events/` 準備
- [ ] `event_metadata.csv` 格式確認
- [ ] 痛點類型清單（數位轉型、資訊安全、流程自動化...）
- [ ] Gemini API 額度檢查
...
- [ ] 溫/冷沉睡分界線（目前 180 天）
- [ ] 交叉銷售規則閾值（Lift > 1.5）
- [ ] 話術生成批次大小（建議每 10 筆 sleep 2 秒）

#### 9.5.8 費用估算（Phase 3 新增部分）

| 模組 | API 呼叫量估算 | 費用（Gemini Flash） |
|------|---------------|---------------------|
| 活動內容解析 | 10-50 個活動 × 1 次 | < $1 |
| 邀約話術生成 | 1,000-5,000 個潛客 × 1 次 | $3-15 |
| **Phase 3 新增合計** | | **$4-16** |

> 加上原有 Phase 3 痛需熱圖執行費用（估計 $10-30），Phase 3 完整執行費用約 **$14-46**
...
| 商機等級分類 | $30–50 | Gemini Flash |
| **Phase 3：痛需熱圖基礎** | **$10–30** | **Gemini Flash** |
| **Phase 3：活動內容解析** ⭐ | **< $1（10-50 個活動）** | **Gemini Flash** |
| **Phase 3：邀約話術生成** ⭐ | **$3–15（1,000-5,000 潛客）** | **Gemini Flash** |
| **Phase 3 小計** | **$14–46** | |
| **總計（Phase 0–M+1+2+商機+3）** | **約 $263–537** | 零人工介入 |

**v5.0 新增說明：**
- 活動內容解析：每個活動 1 次 API 呼叫，支援 PDF/Word/圖片（OCR）
- 邀約話術生成：每個沉睡潛客 1 次 API 呼叫，150-200字個人化話術
- 業務時間節省：100 個潛客 × 5 分鐘 = 省 8.3 小時（話術撰寫）

---

## 十二、時程規劃

### 12.1 Phase 進度總覽（2026-05-04）
...
| 商機等級 | LeadInfo 4,240,000 筆 E/D/C2/C1/B/A 判定 ✅ | W5–W6 | ✅ 完成 |
| Phase 2 | L1–L7 深度標籤（15 Workers，173,376 筆）✅ | W5–W6 | ✅ 完成 |
| **Phase 3** | **痛需熱圖 + 三輸出 + 合約分析 + 活動推薦 + 話術生成** | **W7–W10** | **📋 設計完成，待執行** |
| Phase 5 | 擴展新資料源（客服/會議/外部） | W11–W13 | ⬜ |

### 12.2 甘特圖（v5.0，2026-05-04）

```
任務                          4/01  4/08  4/15  4/22  4/29  5/06▶ 5/13  5/20  5/27  6/03
                               W1    W2    W3    W4    W5    W6    W7    W8    W9    W10
──────────────────────────────────────────────────────────────────────────────────────────
Phase 0：GCP + 問卷 Adapter    ████  ████  ████
Step 0：種子庫建立             ████  ████  ████
Step 1：規則快篩                     ████  ████
Step 2：Embedding+KNN                      ████  ████
Step 3：SC（14筆）→ 180萬筆                      ████
Phase M：聚類+REPORT.html                         ████  ████
Phase 1：法人標籤 206k家               ████  ████  ████
商機等級（424萬，三層pipeline）                   ████  ████  ← ✅ 完成
Phase 2：深度標籤（173k筆）                       ████  ████  ← ✅ 完成
Phase 3-基礎：痛需熱圖                                    ▶NOW  ░░░░
Phase 3-合約分析 ⭐                                       ▶NOW  ░░░░  ░░░░
Phase 3-活動推薦+話術 ⭐                                  ▶NOW  ░░░░  ░░░░  ░░░░

說明：████ 已完成  ▶NOW 當前週（W6，5/06）  ░░░░ 待執行  ⭐ v5.0 新增
```

### 12.3 近期待辦（W6-W10，2026-05-04）

| 優先序 | 任務 | 狀態 |
|--------|------|------|
| **P0** | **Phase 3 基礎執行（痛需熱圖 + 三輸出）** | **⏸️ 下一步** |
| **P0** | **合約資料撈取配置（確認表名、欄位對應）** ⭐ | **⏸️ 待確認** |
| **P1** | **活動資料準備（events/ 資料夾 + event_metadata.csv）** ⭐ | **⏸️ 待準備** |
| P1 | 活動內容解析執行（parse_event_content.py）⭐ | 活動資料準備後 |
| P1 | 沉睡潛客識別與話術生成 ⭐ | 活動解析後 |
| P2 | Phase 3 視覺化升級（Plotly 互動熱圖 → REPORT.html） | Phase 3 執行後 |
| P2 | 資料品質驗證（Phase 2 抽樣檢查、商機等級合理性驗證） | 建議執行 |

**⭐ v5.0 新增重點：**
- 合約續約分析（6種客戶類型分類）
- 活動內容自動解析（Gemini Flash，支援 PDF/Word/圖片）
- 個人化話術生成（150-200字，含行動呼籲）
...
| **`L-Phase3痛需熱圖.ipynb`** ⭐ | **Phase 3 主流程（痛需熱圖 + 三輸出 + 合約整合）** | **📋 設計完成，待執行** |
| **`fetch_contract_data.py`** ⭐ | **合約資料撈取（SQL Server → contract_details/summary.csv）** | **📋 待配置** |
| **`parse_event_content.py`** ⭐ | **活動內容解析（Gemini Flash，PDF/Word/圖片 → event_library.csv）** | **📋 待執行** |
| **`identify_dormant_leads.py`** ⭐ | **沉睡潛客識別 + 活動智能推薦** | **📋 待執行** |
| **`generate_outreach_scripts.py`** ⭐ | **邀約話術生成（Gemini Flash，個人化 150-200字）** | **📋 待執行** |
| **`analyze_renewal_patterns.py`** ⭐ | **續約路徑分析（renewal_sequences, upgrade_paths, cross_sell_matrix）** | **📋 待執行** |

**⭐ v5.0 新增：** Phase 3 合約分析與活動推薦模組（5 個新腳本）
