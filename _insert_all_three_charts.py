...
def insert_all_three_charts(doc_path, output_path):
    """插入三個圖表"""
    ...
    # 圖表 3：人力維運工時分析（在「人力維運工時評估」之後）
    maintenance_chart_path = Path(r"d:\yujui\痛點需求地圖\prompt定版\maintenance_hours_chart.png")
    pos3 = find_section_position(doc, ["人力維運", "維運工作項目", "維運工時"])

    insert_chart(
        doc, pos3,
        maintenance_chart_path,
        "圖表 3：人力維運工時分析（年度工時分布與趨勢）",
        3
    )

    # 保存
    doc.save(output_path)
    print(f"\n[OK] Document updated with 3 charts: {output_path.name}")

if __name__ == "__main__":
    doc_path = Path(r"d:\yujui\痛點需求地圖\prompt定版\推薦系統規劃.docx")
    output_path = doc_path

    print("Inserting all three charts into document...")
    print(f"Document: {doc_path.name}")
    print("-" * 80)

    insert_all_three_charts(doc_path, output_path)
